...
def rules_screen() -> ft.Control:
    return ft.Column(
        [
            heading("Правила Яцзы"),
            card(
                section_label("Как играют"),
                body("Пять шестигранных кубиков. За ход — не больше трёх бросков."),
                body("После первого и второго любые кости можно оставить, остальные кинуть снова. Можно передумать и снова бросить то, что только что держали."),
                body("Результат записывают в одну свободную клетку — после третьего броска или раньше, если комбинация уже есть. Каждую клетку заполняют один раз."),
                body("Если комбинации нет, ставят 0: ход всё равно закрывают. Партия кончается, когда заполнены все клетки. Побеждает большая сумма."),
            ),
            card(
                section_label("Первый этап"),
                _rule("Единицы–шестёрки", "В клетку пишут сумму только этих костей. Пять кубиков, поэтому максимум: 5, 10, 15, 20, 25 и 30."),
                _rule("Премия 35", "Если сумма шести клеток не меньше 63 — плюс 35. Ориентир: по три кости каждой номинации."),
            ),
            card(
                section_label("Второй этап"),
                _rule("Пара", "Две одинаковые кости. Сумма всех пяти кубиков."),
                _rule("Две пары", "Две разные пары. Сумма всех пяти."),
                _rule("Сет", "Три одинаковые. Сумма всех пяти."),
                _rule("Каре", "Четыре одинаковые. Сумма всех пяти."),
                _rule("Малый стрит", "Четыре значения подряд — 30. Например 1-2-3-4 или 3-4-5-6."),
                _rule("Большой стрит", "Пять подряд — 40: 1-2-3-4-5 или 2-3-4-5-6."),
                _rule("Фулл-хаус", "Тройка и пара разных номиналов — 25."),
                _rule("Яцзы", "Все пять одинаковые — 50."),
                _rule("Шанс", "Любой бросок: сумма всех пяти костей."),
                body("Нет комбинации — в клетку ставят 0."),
            ),
        ],
        spacing=12,
        scroll=ft.ScrollMode.AUTO,
...
def _rule(title: str, text: str) -> ft.Control:
    return ft.Column(
        [
